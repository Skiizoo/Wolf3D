...
void			rayCasting(t_mlx *mlx)
{
	...
	if ((cast.y = mlx->user.pov - ANGLE30) < 0)
		cast.y = ANGLE360 + cast.y;
	cast.x = -1;
	while (++cast.x < ANGLE60)
	{
		distHitGrid.x = getHorizontalDist(mlx, cast.y);
		distHitGrid.y = getVerticalDist(mlx, cast.y);
